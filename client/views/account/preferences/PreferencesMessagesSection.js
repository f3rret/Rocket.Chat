...
const PreferencesMessagesSection = ({ onChange, commitRef, ...props }) => {
	...
	const settings = {
		...
		autoDownloadP2P: useUserPreference('autoDownloadP2P'),
				...
				{useMemo(
					() => (
						<Field display='flex' flexDirection='row' justifyContent='spaceBetween' flexGrow={1}>
							<Field.Label>{'Автоприем файлов и папок P2P для всех'}</Field.Label>
							<Field.Row>
								<ToggleSwitch checked={autoDownloadP2P} onChange={handleAutoDownloadP2P} />
							</Field.Row>
						</Field>
					),
					[handleAutoDownloadP2P, autoDownloadP2P],
				)}
				{useMemo(
					() => (
						<Field display='flex' flexDirection='row' justifyContent='spaceBetween' flexGrow={1}>
