...
import { isEmail, isPhone } from '../../../app/utils/lib/isEmail.js';
...
function AccountProfileForm({ values, handlers, user, settings, onSaveStateChange, ...props }) {
	...
	const phoneError = useMemo(
		() => (isPhone(phone) ? undefined : 'Некорректный номер телефона'),
		[phone],
	);
			...
			{useMemo(
					() => (
						<Field>
							...
							{!allowPhoneChange && <Field.Hint>{'Администратор запретил изменять это поле'}</Field.Hint>}
							<Field.Error>{phoneError}</Field.Error>
						</Field>
					),
					[t, phone, handlePhone, allowPhoneChange, phoneError],
				)}
			{useMemo(
				() => (
					<Field>
