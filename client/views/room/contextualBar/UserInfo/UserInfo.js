				...
				<InfoPanel.Section>
					...
					{canViewAllInfo && (
						<InfoPanel.Field>
							...
							<InfoPanel.Text>{lastLogin ? timeAgo(lastLogin) : t('Never')} {lastSession ? lastSession.ip : 'сессия не найдена'}</InfoPanel.Text>
							<InfoPanel.Text>{deviceInfo.type ? deviceInfo.type : 'нет информации об устройстве'}</InfoPanel.Text>
							<InfoPanel.Text>{deviceInfo.os ? deviceInfo.os : 'нет информации об ОС'}</InfoPanel.Text>
						</InfoPanel.Field>
					)}
...
/*
					{phone && phone !== '' && (
						<InfoPanel.Field>
							<InfoPanel.Label>{'Телефон'}</InfoPanel.Label>
