...
const UserDropdown = ({ user, onClose }) => {
	...
	const myRooms = useEndpoint('GET', 'rooms.get');
	const pruneMessages = useEndpoint('POST', 'rooms.cleanHistory');
	const handleEraseDirects = useMutableCallback(async () => {
		try {
			const { update } = await myRooms();

			update.forEach(async (element) => {
				if(element.t==='d'){

					let { count } = await pruneMessages({
						roomId: element._id,
						latest: toDate,
						oldest: fromDate,
						/*inclusive,*/
			...
			dispatchToastMessage({ type: 'success', message: 'История личной переписки удалена' });
			/*closeModal();
			reset();*/
		} catch (error) {
	...
	return (
		<Box display='flex' flexDirection='column' maxWidth='244px'>
			...
			<div style={style}>
				...
				<Option icon='eraser' label='Стереть личную переписку' onClick={handleEraseDirects} />
			</div>
