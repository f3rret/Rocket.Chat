...
const RoomMenu = ({ rid, unread, threadUnread, alert, roomOpen, type, cl, name = '' }) => {
	...
	const rmsub = useUserSubscription(rid, fields);
	const canFavorite = useSetting('Favorite_Rooms');
	const canAutoDownload = true;// useSetting('autoDownloadP2P');
	const isFavorite = (rmsub != null ? rmsub.f : undefined) != null && rmsub.f;
	const isAutoDownload =(rmsub != null ? rmsub.autoDownloadP2P : undefined) != undefined && rmsub.autoDownloadP2P;
	...
	const toggleAutoDownload = useMethod('toggleAutoDownload');
	...
	const handleToggleRead = useMutableCallback(async () => {
		try {
			...
			RoomManager.close(rmsub.t + rmsub.name);
	...
	const handleToggleAutoDownload = useMutableCallback(async () => {
		try {
			await toggleAutoDownload(rid, !isAutoDownload);
		} catch (error) {
			dispatchToastMessage({ type: 'error', message: error });
		}
	});
	...
	const menuOptions = useMemo(
		() => ({
			...
			...(canAutoDownload && {
				toggleAutoDownload: {
					label: {
						label: isAutoDownload ? 'Отключить автоприем p2p' : 'Включить автоприем p2p',
						icon: isAutoDownload ? 'star-filled' : 'star',
					},
					action: handleToggleAutoDownload,
				},
			}),
		}),
		[
