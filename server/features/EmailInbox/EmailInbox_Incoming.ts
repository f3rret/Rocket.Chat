...
function getRoomId(roomName:any) {
	roomName = roomName.replace('#', '');
	const room = Rooms.findOneByNonValidatedName(roomName, { fields: { _id: 1, t: 1 } });
...
export async function onEmailReceived(email: ParsedMail, inbox: string, department?: string): Promise<void> {
	...
	if(department && department.startsWith('#')){
		rid=getRoomId(department);
		if(!rid)return;
	}
	else{
		const room=Users.findOneByUsername(department);
		if(!room)return;
		rid=[room._id, 'rocket.cat'].sort().join('');
	}
