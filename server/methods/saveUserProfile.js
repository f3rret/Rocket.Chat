...
import { saveUserIdentity, saveUserPhone } from '../../app/lib/server/functions/saveUserIdentity';
...
function saveUserProfile(settings, customFields) {
	...
	if (settings.phone != null) {
		if (typeof settings.phone !== 'string' || settings.phone.length > 50 || !/^[\d,]{1,11}$/.test(settings.phone)) {
			throw new Meteor.Error('error-invalid-field', 'phone', {
				method: 'saveUserProfile',
			});
		}

		saveUserPhone(user._id, settings.phone.trim());	
