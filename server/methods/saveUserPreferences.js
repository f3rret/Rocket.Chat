...
Meteor.methods({
	saveUserPreferences(settings) {
		const keys = {
			...
			autoDownloadP2P: Match.Optional(Boolean),
		...
		const {
			...
			autoDownloadP2P: oldAutoDownloadP2P,
		...
		Meteor.defer(() => {
			if(oldAutoDownloadP2P !== settings.autoDownloadP2P){
				Subscriptions.setAutoDownloadByUserId(user._id, settings.autoDownloadP2P);
			}
