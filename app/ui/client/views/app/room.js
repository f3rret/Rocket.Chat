...
export const dropzoneEvents = {
	...
	async 'dropped .dropzone-overlay'(event, instance) {
		...
		let forceZip=undefined;
		for(let i=0; i<e.dataTransfer.items.length; i++){
			if(forceZip==undefined && e.dataTransfer.items[i].webkitGetAsEntry().isDirectory){
				forceZip=e.dataTransfer.items[i].webkitGetAsEntry().name;
				break;
			}
		}
		...
		for(let i=0; i<items.length; i++){
			var file = await getFile(items[i]);
		...
		Session.set('zipping', []);
		return instance.onFile && instance.onFile(filesToUpload, forceZip);
	},
};
...
Meteor.startup(() => {
	Template.roomOld.events({
		...
		'click .zipping-progress-close'(e) {
			e.preventDefault();
			Session.set('zipping', []);
		},
	...
	Template.roomOld.onCreated(function() {
		...
		this.onFile = (filesToUpload, forceZip) => {
			fileUpload(filesToUpload, chatMessages[rid].input, { rid }, forceZip);
		};
