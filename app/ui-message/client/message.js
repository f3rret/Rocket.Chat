...
Template.message.helpers({
	...
	isP2Pui() {
		const { msg } = this;
		return msg.p2p === true && msg.sys !== true;
	},
	isP2Psys() {
		const { msg } = this;
		return msg.p2p === true && msg.sys === true;
	...
	hideActionLinks() {
		...
		const { room, msg, u } = this;
		
		if(msg.t==='jitsi_call_started'){
			if(msg.u._id===u._id){
				/*const currentTime = new Date().getTime();
				const r = Rooms.findOne({ _id: room._id });
				if(room && room.jitsiTimeout && !msg.closed){
					const jitsiTimeout = new Date(room.jitsiTimeout).getTime();
					
					if (jitsiTimeout < currentTime) {
						console.log('send updatemessage ', msg._id);
						call('updateMessage', {
							_id: msg._id,
							rid: room._id,
							msg: 'Видеозвонок завершен',
							actionLinks: [],
							closed: true
						});
					}
