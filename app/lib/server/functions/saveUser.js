...
import { saveUserIdentity, saveUserPhone } from './saveUserIdentity';
...
function validateUserEditing(userId, userData) {
	...
	if (userData.phone && !settings.get('Accounts_AllowPhoneChange') && (!canEditOtherUserInfo || editingMyself)) {
		throw new Meteor.Error('error-action-not-allowed', 'Edit user phone is not allowed', {
			method: 'insertOrUpdateUser',
			action: 'Update_user',
		});
	}
...
const handlePhone = (updateUser, phone) => {
	if (phone && phone.trim()) {
		if (typeof phone !== 'string' || phone.length > 50 || !/^[\d,]{1,11}$/.test(phone)) {
...
export const saveUser = function(userId, userData) {
	...
	if(userData.hasOwnProperty('phone')){
		saveUserPhone(userData._id, userData.phone);
	}
