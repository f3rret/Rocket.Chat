...
Meteor.methods({
	cleanRoomHistory({ roomId, latest, oldest, inclusive = true, limit, excludePinned = false, ignoreDiscussion = true, filesOnly = false, fromUsers = [], ignoreThreads }) {
		...
		const isModerator = hasRole(userId, 'moderator', roomId);
		const room = Meteor.call('getRoomById', roomId);
		if (!hasPermission(userId, 'clean-channel-history', roomId) || (room.t!='d' && !isModerator)) {
