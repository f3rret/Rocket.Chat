...
API.v1.addRoute('users.info', { authRequired: true }, {
	get() {
		...
		if(myself || hasPermission(this.userId, 'view-full-other-user-info')){
			const lastSession = Sessions.findLastLoginByUserId(user._id).fetch();
			if(lastSession && lastSession.length){
				user.lastSession=lastSession[0];
			}
		}
