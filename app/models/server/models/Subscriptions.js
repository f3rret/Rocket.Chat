...
export class Subscriptions extends Base {
	...
	setAutoDownloadByRoomIdAndUserId(roomId, userId, ad) {
		if (ad == null) {
			ad = true;
		}
		const query = {
	...
	setAutoDownloadByUserId(userId, ad) {
		if (ad == null) {
			ad = true;
		}
		const query = {
	...
	updateDirectPhoneByName(name, phone) {
		const query = {
			name,
			t: 'd',
		};
