	...
	const Cls = class {
		static initClass() {
			...
			this.prototype.computation = Tracker.autorun(() => {
				...
				try{
					const subs = ChatSubscription.find({ autoDownloadP2P: true }).fetch();

					for(var i=0; i<subs.length; i++){
						var s=subs[i];

						if(s.rid){
							if(!RoomManager.getOpenedRoomByRid(s.rid)){
								if(!msgStream.p2pListen){
									msgStream.p2pListen={};
								}

								if(!msgStream.p2pListen[s.rid]){
									msgStream.p2pListen[s.rid]=[1];
									msgStream.on(s.rid, async (msg)=>{
										try{									
											if(msg.p2p && msg.signal && msgStream.p2pListen[s.rid].indexOf(msg._id)===-1){
												if(!RoomManager.getOpenedRoomByRid(s.rid) && s && s.autoDownloadP2P===true){ //если не отключили автоприем и не открыт рум
													msgStream.p2pListen[s.rid].push(msg._id);
													await processP2P(msg, true);
												}									
											}
										}
					...
					console.log('init autodownload error:', e);
				}
