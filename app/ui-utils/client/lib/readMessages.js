...
export const readMessage = new class extends Emitter {
	...
	refreshUnreadMark(rid) {
		...
		let room = RoomManager.openedRooms[subscription.t + subscription.name];
		if(room == null){
			room = RoomManager.openedRooms[subscription.t + rid];
		}
		if (room == null) {
			return;
		}
		...
		.on('click', (e) => {
			try{
				if(e.target && e.target.parentNode){
					if(e.target.parentNode.getAttribute('data-unread-text') || e.target.parentNode.className.indexOf('first-unread')>-1){
						const rid = Session.get('openedRoom');
						if (!rid) {
							return;
						}
